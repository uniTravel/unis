...
impl<A, R, S, L, F> Dispatch<A, R, S, L> for F
where
    A: Aggregate,
    R: Replay<A = A>,
    S: Stream<A = A>,
    ...
    ) -> Result<(A, A, Vec<u8>), DomainError>,
{
    ...
    fn dispatch(
        ...
        self(agg_id, com_data, caches, replayer, stream, loader)
    }
}
...
pub struct Aggregator<A, D, R, S, L>
where
    A: Aggregate,
    D: Dispatch<A, R, S, L>,
    R: Replay<A = A>,
...
impl<A, D, R, S, L> Aggregator<A, D, R, S, L>
where
    A: Aggregate + Send + 'static,
    D: Dispatch<A, R, S, L> + Send + 'static,
                ...
                Some(task) = rx.recv() => {
                    match task {
                        Task::Com { agg_id, com_id, com_data, reply_tx } => {
                            match dispatcher.dispatch(agg_id, com_data, &mut caches, &replayer, &stream, &loader) {
