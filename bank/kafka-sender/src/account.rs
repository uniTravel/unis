...
pub async fn create<F>(
    ...
    let res = svc.apply(key, AccountCommand::Create(com)).await;
    unis::into(res, &lang)
}
...
pub async fn verify<F>(
    ...
    let res = svc.apply(key, AccountCommand::Verify(com)).await;
    unis::into(res, &lang)
}
...
pub async fn approve<F>(
    ...
    let res = svc.apply(key, AccountCommand::Approve(com)).await;
    unis::into(res, &lang)
}
...
pub async fn limit<F>(
    ...
    let res = svc.apply(key, AccountCommand::Limit(com)).await;
    unis::into(res, &lang)
}
