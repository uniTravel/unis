...
#[rstest]
#[tokio::test]
async fn write_with_agg_topic(
    #[future(awt)] _init: (),
    #[future(awt)] context: (Arc<App>, Writer),
) {
...
#[rstest]
#[tokio::test]
async fn write_without_agg_topic(
    #[future(awt)] _init: (),
    #[future(awt)] context: (Arc<App>, Writer),
) {
    let (app, stream) = context;
...
#[rstest]
#[tokio::test]
async fn respond_to_stream(#[future(awt)] _init: (), #[future(awt)] context: (Arc<App>, Writer)) {
    let (app, stream) = context;
    let agg_type = note::Note::topic();
